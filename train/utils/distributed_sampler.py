...
from train.utils.tools import bucketize
...
class DistributedBucketSampler(Sampler[_T_co]):
    def __init__(
        ...
        **kwargs
        ...
        if shuffle:
            g = torch.Generator()
            g.manual_seed(self.seed + self.epoch)
            self.bucketized_indices = [
                self.bucketized_indices[i] for i in \
                torch.randperm(len(self.bucketized_indices), generator=g).tolist()
            ]

        if self.drop_last and len(self.bucketized_indices) % self.num_replicas != 0:  # type: ignore[arg-type]
            # Split to nearest available length that is evenly divisible.
            # This is to ensure each rank receives the same amount of data when
            # using this Sampler.
            self.num_samples = math.ceil(
                (len(self.bucketized_indices) - self.num_replicas) / self.num_replicas  # type: ignore[arg-type]
            )
        else:
            self.num_samples = math.ceil(len(self.bucketized_indices) / self.num_replicas)  # type: ignore[arg-type]
    ...
    def __iter__(self) -> Iterator[_T_co]:
        if self.shuffle:
            ...
            g = torch.Generator()
            g.manual_seed(self.seed + self.epoch)
            indices = torch.randperm(len(self.bucketized_indices), 
                                     generator=g).tolist()
        else:
            indices = list(range(len(self.bucketized_indices)))
